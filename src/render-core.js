/* Shared rendering core for the orb window.
 *
 * Pixel primitives, three-tone body shading, faces, confetti, and the per-state motion
 * that every skin shares. Skins (skin-*.js) draw the creature itself; this file is
 * everything they have in common so they stay visually consistent.
 */
...
/**
 * Eyes + mouth + cheeks, sized for a body of radius `r`.
 *
 * Pixel art doesn't scale smoothly, so below a certain size the face has to lose
 * detail rather than shrink: a 4px eye sprite plus a mouth simply doesn't fit on a
 * 12px body. Small bodies get two dots and nothing else.
 */
function drawFaceSized(octx, cx, eyeY, kind, eyeDx, r) {
  if (r < 8) {
    // Minimal face: a pair of dots, sized so they stay inside the silhouette.
    const gap = Math.max(2, Math.round(r * 0.42));
    const size = r < 6 ? 1 : 2;
    const closed = kind === "closed";
    const c = Math.round(cx);
    px(octx, c - gap - 1, eyeY + 1, size, closed ? 1 : size, P.ink);
    px(octx, c + gap - 1, eyeY + 1, size, closed ? 1 : size, P.ink);
    return;
  }
  drawFace(octx, cx, eyeY, kind, eyeDx, Math.max(5, Math.round(r * 0.47)), r >= 12);
}

/** Eyes + mouth + cheeks, centred on `cx`. */
function drawFace(octx, cx, eyeY, kind, eyeDx = 0, spread = 7, blush = true) {
