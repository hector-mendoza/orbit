 ...
 * and the body's stretch carry the state: a calm ripple while working, a fast agitated
 * one when waiting (rearing up tall), and a slow drift when idle.
 */
...
SKINS.ghost = (() => {
  ...
  /** Dome on top, straight sides, scalloped hem — drawn span by span. */
  function body(octx, cx, cy, rx, ry, cfg, wavePhase, waveAmp) {
    ...
    for (let row = top; row < hemY + 7; row++) {
      ...
      if (row >= hemY) {
        ...
        for (let x = x0; x < x1; x++) {
          ...
          if (depth < 1 + bump * (4 + waveAmp)) {
            px(octx, x, row, 1, 1, x > cx + rx * 0.35 ? cfg.rim : cfg.mid);
          }
        }
        continue;
      }
  ...
  return {
    label: "Ghost",
    burstOrigin: (scale) => [CX, CY - BODY_RY * scale - 4],
    ...
    draw(octx, cfg, state, now, dt, pose) {
      ...
      groundShadow(octx, CX, 58, Math.round(22 * sc), 2 + Math.abs(Math.round(float)));

      const rx = (BODY_RX * sc) / Math.sqrt(stretch);
      const ry = BODY_RY * sc * stretch;
      body(octx, cx, cy, rx, ry, cfg, now / waveSpeed, waveAmp);

      drawFaceSized(octx, cx, Math.round(cy - ry * 0.18), pose.face, pose.eyeDx, ry);
    },
  };
})();
